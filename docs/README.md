...
| Tài liệu | Nội dung |
|----------|----------|
| [architecture.md](./architecture.md) | Kiến trúc tổng thể, luồng khởi động, routing, cây thư mục, code-splitting |
| [data-layer.md](./data-layer.md) | `src/data/*` — nguồn sự thật duy nhất cho hồ sơ, kinh nghiệm, tech stack, dự án |
...
| [i18n.md](./i18n.md) | Cách hoạt động đa ngôn ngữ, quy ước key, script kiểm tra |
| [styling.md](./styling.md) | Design system `pf-*`, tokens, scope CSS của module |
| [external-apis.md](./external-apis.md) | Tích hợp Codeforces, GitHub calendar, PDF resume |
| [personal-data.md](./personal-data.md) | Nơi lưu thông tin cá nhân, cần sửa ở đâu khi cập nhật |
| [development.md](./development.md) | Setup, lệnh, env, build & deploy (Vite) |

## Tóm tắt nhanh

- **Stack**: Vite 5 + React 18, React Router v6, react-i18next, Tailwind (chỉ cho
  biểu đồ Codeforces) + design system CSS custom `pf-*`, recharts,
  react-github-calendar, react-icons, lucide-react, @xyflow/react.
- **Trang portfolio**: Home (`/`), About (`/about`), **Stack (`/stack`)**,
  Projects (`/project`), Resume (`/resume`).
...
- **Nội dung là dữ liệu**: hồ sơ / kinh nghiệm / tech stack / dự án khai báo
  trong [`src/data/`](../src/data), text hiển thị nằm trong i18n.
